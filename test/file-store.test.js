...
describe('Test file cookie store', function () {
    ...
    beforeEach(function (done) {
        FS.writeFileSync(
            COOKIES_TEST_FILE2,
            FS.readFileSync(COOKIES_TEST_FILE)
        );
            ...
            cookie_store.findCookie(
                ...
                function (err, cookie) {
                    try {
                        ...
                        expect(cookie.value).to.be(
                            'fUW2oGT39LOjTSJEvoSq+tfDjwQ1hO+QEEvx+D0eUDjWGotyXIFKVtj1DJi+k50tivsK3bmd8VlLjNm1XbmORTf/Xylom0EWEyPMtJuOqjqKUnkBr3cyww=='
                        );
        ...
        it('should save cookie', function (done) {
            cookie_store.findCookies(
                ...
                function (err, cookies) {
                    ...
                    var cookie_store2 = new FileCookieStore(
                        COOKIES_TEST_FILE_NEW
                    );
        ...
        it('should insert new cookie', function (done) {
            ...
            Q.nbind(
                ...
                .then(function (cookie) {
                    ...
                    expect(Math.round(cookie.expires.getTime() / 1000)).to.be(
                        Math.round(expire.getTime() / 1000)
                    );
    ...
    describe('#removeCookie', function () {
        it('should remove cookie', function (done) {
            var removeCookie = Q.nbind(
                cookie_store.removeCookie,
                cookie_store,
                '.twitter.com',
                '/',
                'guest_id'
            ...
            removeCookie()
                .then(function (cookie) {
                    ...
                    return Q.nbind(cookie_store2.findCookie, cookie_store2)(
                        '.twitter.com',
                        '/',
                        'guest_id'
                    );
        ...
        it('should remove all domain cookies', function (done) {
            ...
            Q.nbind(cookie_store.findCookies, cookie_store)(test_domain, null)
                .then(function (cookies) {
                    ...
                    return Q.nbind(cookie_store.removeCookies, cookie_store)(
                        test_domain,
                        null
                    );
                })
    ...
    describe('#CookieJar', function () {
            ...
            Q.nbind(
                ...
                .then(function (cookies) {
                    ...
                    return Q.nbind(
                        cookie_jar.getCookies,
                        cookie_jar
                    )('http://www.alibaba.com/path/test');
                })
                .then(function (cookies) {
                    expect(cookies).to.be.a(Array);
        ...
        it('should put cookie in CookieJar', function (done) {
            ...
            Q.nbind(cookie_jar.setCookie, cookie_jar)(
                cookie,
                'http://setcookietest.com/'
            )
                .then(function (cookie) {
                    expect(cookie).to.be.a(TOUGH.Cookie);
                    return Q.nbind(
                        cookie_jar.getCookies,
                        cookie_jar
                    )('http://setcookietest.com/test/path');
                })
                .then(function (cookies) {
                    expect(cookies).to.be.a(Array);
                    expect(cookies).to.have.length(1);
        ...
        it('should save cookie into file from CookieJar', function (done) {
            ...
            Q.nbind(cookie_jar.setCookie, cookie_jar)(
                ...
                .then(function (cookie) {
                    expect(cookie).to.be.a(TOUGH.Cookie);
                    var cookie_jar2 = new TOUGH.CookieJar(
                        new FileCookieStore(COOKIES_TEST_FILE2)
                    );
                    return Q.nbind(
                        cookie_jar2.getCookies,
                        cookie_jar2
                    )('http://setcookietest.com/test/path');
                })
                .then(function (cookies) {
            ...
            Q.nbind(cookie_jar.setCookie, cookie_jar)(
                ...
                .then(function (cookie) {
                    ...
                    return Q.nbind(
                        cookie_jar.getCookies,
                        cookie_jar
                    )('https://setcookietest.com/test/path');
                })
                .then(function (cookies) {
                    expect(cookies).to.be.a(Array);
                    expect(cookies).to.have.length(1);
        ...
        it('should save "host only" cookies correctly', function (done) {
            ...
            var cookie_store2 = new FileCookieStore(COOKIES_TEST_FILE_NEW),
                cookie_jar2 = new TOUGH.CookieJar(
                    new FileCookieStore(COOKIES_TEST_FILE_NEW)
                );

            Q.all(fns)
                .spread(function (cookies1, cookies2, cookies3, cookies4) {
                    ...
                    var all_cookies = cookies1.concat(
                            cookies1,
                            cookies2,
                            cookies3,
                            cookies4
